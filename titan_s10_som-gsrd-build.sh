...
LINUX_SOCFPGA_BRANCH=titan-s10-som-$LINUX_VER-lts
...
UBOOT_SOCFPGA_BRANCH=titan-s10-som-$UBOOT_VER$UBOOT_REL
...
bitbake_image() {
	pushd $WORKSPACE/$MACHINE-$IMAGE-rootfs > /dev/null
		...
		echo -e "\n[INFO] Clean up previous bootloader build if any"
		bitbake virtual/bootloader -c cleanall
		echo -e "\n[INFO] Clean up previous bitstream build if any"
		bitbake virtual/bitstream -c cleanall
		echo -e "\n[INFO] Clean up previous dtb build if any"
		bitbake virtual/dtb -c cleanall
...
package() {
	...
	mkdir -p $STAGING_FOLDER/u-boot-$MACHINE-$IMAGE-atf
	ub_cp_destination=$STAGING_FOLDER/u-boot-$MACHINE-$IMAGE-atf
	...
	pushd $WORKSPACE/$MACHINE-$IMAGE-rootfs/tmp/deploy/images/$MACHINE/ > /dev/null
		cp -vrL ${MACHINE}_rbf/ $STAGING_FOLDER/. || echo "[INFO] File core.rbf not found for this build configuration."
	popd > /dev/null
