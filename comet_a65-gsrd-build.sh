...
LINUX_SOCFPGA_BRANCH=comet-a65-revB-$LINUX_VER-lts
...
UBOOT_SOCFPGA_BRANCH=comet-a65-revB-$UBOOT_VER$UBOOT_REL
...
ATF_BRANCH=comet-a65-revB-$ATF_VER
...
#------------------------------------------------------------------------------------------#
	pushd $WORKSPACE > /dev/null
		...
		bitbake-layers add-layer ../meta-terasic
		...
		echo 'CORE_IMAGE_EXTRA_INSTALL += "openssh gdbserver mtd-utils net-tools usbutils memtester"' >> conf/site.conf
			
		
		echo "require conf/machine/$MACHINE-gsrd.conf" >> conf/site.conf
		# Linux
		echo 'PREFERRED_PROVIDER_virtual/kernel = "linux-socfpga-lts-tr"' >> conf/site.conf
		echo "PREFERRED_VERSION_linux-socfpga-lts-tr = \"`cut -d. -f1-2 <<< "$LINUX_VER"`%\"" >> conf/site.conf
		...
		# U-boot
		echo 'PREFERRED_PROVIDER_virtual/bootloader = "u-boot-socfpga-tr"' >> conf/site.conf
		echo "UBOOT_CONFIG:${MACHINE} = \"$UB_CONFIG\"" >> conf/site.conf
		echo "PREFERRED_VERSION_u-boot-socfpga-tr = \"$UBOOT_VER%\"" >> conf/site.conf
		echo "UBOOT_BRANCH = \"$UBOOT_SOCFPGA_BRANCH\"" >> conf/site.conf
		# ATF
		echo "PREFERRED_VERSION_arm-trusted-firmware-tr = \"`cut -d. -f1-2 <<< "$ATF_VER"`\"" >> conf/site.conf
...
bitbake_image() {
	pushd $WORKSPACE/$MACHINE-$IMAGE-rootfs > /dev/null
		...
		bitbake gsrd-sdcard-console gsrd-emmc-console 2>&1
...
bitbake_esdk() {
    pushd $WORKSPACE/$MACHINE-$IMAGE-rootfs > /dev/null
		...
		echo -e "\n[INFO] Start bitbake process for target config.."
		bitbake gsrd-sdcard-console gsrd-emmc-console -c populate_sdk_ext 2>&1
...
package() {
	...
	pushd $WORKSPACE/$MACHINE-$IMAGE-rootfs/tmp/work/$MACHINE-poky-*/u-boot-socfpga-tr/v20*/build/*defconfig/
	...
	pushd $WORKSPACE/$MACHINE-$IMAGE-rootfs/tmp/deploy/images/$MACHINE/ > /dev/null
		...
		cp -vL ${MACHINE}_uboot.txt $ub_cp_destination
	popd > /dev/null
	...
	pushd $STAGING_FOLDER
		...
		tar cvzf sdimage.tar.gz gsrd_sdcard_console-$MACHINE.wic
		md5sum sdimage.tar.gz > sdimage.tar.gz.md5sum
		
		# Generate emmcimage.tar.gz
		tar cvzf emmcimage.tar.gz gsrd_emmc_console-$MACHINE.wic
		md5sum emmcimage.tar.gz > emmcimage.tar.gz.md5sum
